#!/usr/bin/env python3
"""Validate the exact constitutional authority and its protected-operation boundary."""
...
EXPECTED_ROLES = {
    "canonical-technical-constitution",
...
def validate(root: Path) -> list[str]:
    ...
    protected = set(data.get("protected_operations_require_explicit_operator_authorization", []))
    if protected != EXPECTED_PROTECTED_OPERATIONS:
    ...
    roles = {item.get("role") for item in documents}
    ...
    for item in documents:
        ...
        if not isinstance(rel, str) or not rel:
            errors.append("constitutional document path is missing")
            continue
        ...
        if actual != expected:
            errors.append(f"hash mismatch for {rel}: {actual} != {expected}")
    ...
    technical_hash = technical_doc.get("sha256")
    if technical_hash and technical_hash not in product:
